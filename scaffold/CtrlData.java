package <ScaffoldData>CtrlData.ClassPkgName</ScaffoldData>;
...
import <ScaffoldData>CtrlData.EntityPkg</ScaffoldData>.<ScaffoldData>CtrlData.EntityName</ScaffoldData>;

/**
 * <ScaffoldData>CtrlData.ModuleName</ScaffoldData>数据输出控件
 * */
public class CtrlData extends DataOutforJSONCtrl {
    ...
    public String query(IWebInvokeParam webInvokeParam){
        try{
            ...
            SqlWhereWithParamTools sqlWhere = SqlWhereWithParamTools.create(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class);

            //增加应用编码限制
            if(StringUtils.isNotBlank(webInvokeParam.getSysAppId())){
                sqlWhere.and(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.FieldName_SysAppId,webInvokeParam.getSysAppId());
            }
            ...
            //执行查询
            List<<ScaffoldData>CtrlData.EntityName</ScaffoldData>> datas = SimpleJPABuilderPkg.builder(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class).findEntitys(sqlWhere);

            //格式化输出
            return FormatObjectDataToString.dataBuilder(this, <ScaffoldData>CtrlData.EntityName</ScaffoldData>.class, webInvokeParam).formatDataSetToJson(datas);
        }catch (Exception e){
    ...
    public String add(IWebInvokeParam webInvokeParam) {
        ...
        try{
            ...
            //加载新的数据
            <ScaffoldData>CtrlData.EntityName</ScaffoldData> dataEntity = DataCtrlSessionObj.builder(webInvokeParam).findAndCreateEntity(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class);
            dataEntity.setSysAppId(webInvokeParam.getSysAppId());

            //格式化输出
            return FormatObjectDataToString.dataBuilder(this, <ScaffoldData>CtrlData.EntityName</ScaffoldData>.class, webInvokeParam).formatDataSetToJson(dataEntity);
        }catch (Exception e){
    ...
    public String edit(IWebInvokeParam webInvokeParam) {
        ...
        try{
            ...
            //加载新数据
            <ScaffoldData>CtrlData.EntityName</ScaffoldData> dataEntity = DataCtrlSessionObj.builder(webInvokeParam).findAndLoadByLongKey(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class, currentData);
    ...
    public String save(IWebInvokeParam webInvokeParam) {
        ...
        try{
            ...
            //数据实体
            <ScaffoldData>CtrlData.EntityName</ScaffoldData> dataEntity = sessionObj.findEntity(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class);
            if (dataEntity == null) {
    ...
    public String delete(IWebInvokeParam webInvokeParam) {
        ...
        try {
            //找到要删除的数据实体对象
            String currentData = WebCommonUtilsFunction.getCurrentData(webInvokeParam);
            <ScaffoldData>CtrlData.EntityName</ScaffoldData> dataEntity = SimpleJPABuilderPkg.builder(<ScaffoldData>CtrlData.EntityName</ScaffoldData>.class).findEntity(Long.parseLong(currentData));
            if(dataEntity==null){
