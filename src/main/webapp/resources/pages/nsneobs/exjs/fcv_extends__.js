	...
	$.fn.extend({
		"FinderColumnView": function(options) {
			options = $.extend({
				...
				drawColumnRowView: function(rowObj) {
					...
					sb.append('<li data-fcvrowid="' + rowObj.id + '" data-fcvname="'+rowObj.name+'" data-fcvtype="'+strtype+'" data-fcvdomain="'+strdomain+'" class="list-group-item finder-columnview-list-group-item" ');
				...
				buildColumnView: function(parentid, jsonArrayObj, voptions,coltype,coldomain) {
					...
					if(jsonArrayObj == null || jsonArrayObj.lenght == 0) {
						...
						$.each(jsonArrayObj, function(index, obj) {
							sb.append(voptions.drawColumnRowView(obj));
						});
			...
			that.addRowToTdCol = function(parentId, rowObj) {
				...
				if(tdobj != null) {
					tdobj.find('.finder-columnview-list-group').append(globalOptions.drawColumnRowView(rowObj));
